...
$contributorName = implode(', ', fn($r) => $r->name, $contributor);
...
<?php endforeach; ?>
			...
			<?php if (!in_array('cacke-in-tuhtci', $myCategorySlugs)) : ?>
...
<?php 	endif; ?>
			...
			<?php endif; // not strip title ?>
<?php 
			if (
				(!in_array('Kritika, komentar', $myCategoryNames) && !in_array('Trampolin', $myCategoryNames) && !in_array('Robni zapisi', $myCategoryNames)) && !in_array('Čačke in tuhtci', $myCategoryNames) ||
...
?>
			...
			<?php elseif (in_array('Čačke in tuhtci', $myCategoryNames)) :?>
			<?php if (has_post_thumbnail($post->ID)) : ?>
			<?php $thumbnailURL = wp_get_attachment_url(get_post_thumbnail_id($post->ID)); ?>
			<?php $thumbnailMeta =  wp_get_attachment_metadata(get_post_thumbnail_id($post->ID)); ?>
			<?php $thumbnailIsPortrait = ($thumbnailMeta['height'] > $thumbnailMeta['width'] ? 1 : 0); ?>
			...
			<div id="authorPhotoSidebar" class="center myFeaturedImage <?php echo /*($thumbnailIsPortrait ? 'portraitContainer full-vh' : 'landscapeContainer ')*/ ' full-vh'; //čačke rabijo v bistvu vedno zgolj višino ekrana, sicer so prevelike ?>" style="background-image: url(<?php echo $thumbnailURL; ?>); background-size: contain; background-repeat: no-repeat; background-position: center">
			</div>
			</div>
			<?php endif;  //if thumbnail ?>
			<?php endif; //if not kritika ?>


			<?php if (in_array('Čačke in tuhtci', $myCategoryNames)) : ?>
			<div id="article-title">
				<h1 class="<?php echo $titleSize; ?> leading-tight bold display-font" style=""><?php the_title(); ?></h1>
				<?php if (isset($mySubtitle)) : ?>
				<h2 class="sans font-size-2 gray small-top-margin light inline-block two-three center-margins" style="border-top: 1px solid; padding-top: 0.5em"><?php echo get_post_meta($post->ID, 'mysubtitle', true) ?></h2>
